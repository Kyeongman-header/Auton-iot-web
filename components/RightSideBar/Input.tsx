...
import { addmachine } from '../../redux/machine_slices';
...
export default function Input({machines, handlemachinessearched} : MachineListProperty){
    return (
      <div className="row-start-3 row-end-4 translate duration-200 grid grid-cols-[13] h-[50px] min-w-fit">
          <input
          className="rounded border-green-800 border-[6px] hover:border-green-600 focus:border-green-500  w-full col-start-2 col-end-11"
            ...
            onChange={(event) => {
              machines.map((machine : Machine, index : number) =>{
                ...
                if ((machine.user!=undefined && machine.user?.includes(event.target.value.toString())) || (machine.car_number.includes(event.target.value.toString())) || (machine.id.includes(event.target.value.toString())) )
                {
